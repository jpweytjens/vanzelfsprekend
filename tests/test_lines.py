...
def test_stack_preserves_index_order_of_unsorted_input():
    ...
    heights = np.array([10.0, 10.0])
    np.testing.assert_allclose(placement.stack(desired, heights, 2.0), [50.0, 0.0])


def test_stack_empty_and_single():
    assert placement.stack(np.array([]), np.array([]), 2.0).size == 0
    np.testing.assert_allclose(
        placement.stack(np.array([7.0]), np.array([10.0]), 2.0), [7.0]
    )
