...
with plt.style.context("vanzelfsprekend"):  # geometry knob
    ...
    ax.plot(...)
    vzs.apply(ax, frame="data")  # frame knob: trim, grey, declutter
    fig.savefig("figure.png")
```

Keep the drawing, `apply` and save inside the `style.context`: matplotlib reads these defaults when it renders, not when you call `plot`, so leaving the context early would drop them before the figure is drawn.
